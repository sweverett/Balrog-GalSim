...
def get_catalogs(path = '../Data/', tilename = 'DES0347-5540', re_id = '0', minsep = 0., stars=False, ratag = 'ra', dectag = 'dec'):
    if type(re_id) is not type(''):
        re_id = str(re_id)
    if stars is False:
        truth_catalog = fitsio.read(path+tilename+'_'+re_id+'/'+tilename+'_'+re_id+'_balrog_truth_cat_gals.fits')
    else:
        truth_catalog = fitsio.read(path+tilename+'_'+re_id+'/'+tilename+'_'+re_id+'_balrog_truth_cat_stars.fits')
    if minsep > 0.:
        truth_catalog = remove_close_pairs(truth_catalog,radius=minsep,ratag=ratag,dectag=dectag)
        
    meas_catalog = fitsio.read(path+tilename+'_'+re_id+'/'+tilename+'_mof.fits')
...
def make_plots(truth = None, meas_matched = None, truth_matched = None, sep = None, filetag = '', bandind = 1):
    ...
    fig.savefig('cm_flux-'+filetag+bands[bandind])
    ...
    fig.savefig('completeness-'+filetag+bands[bandind])
    ...
    ax1.legend(loc='best')
    fig.savefig('t_vs_mag_color-coded-by-delta-mag'+bands[bandind])
...
def main(argv):
    ...
    for i in xrange(4):
        make_plots(truth=truth_gal_catalog, meas_matched = meas_gal_matched, truth_matched = truth_gal_matched, sep = sep,filetag='',bandind=i)
